...
export async function PATCH(request, { params }) {
    ...
    const updated = await prisma.advanceQuestion.update({
        where: { id: params.id },
        data: updates,
        include: includeConfig,
    });
...
export async function DELETE(request, { params }) {
    ...
    await prisma.advanceQuestion.delete({ where: { id: params.id } });
    ...
    if (deleteAttachments && fileKeys.length > 0) {
        ...
        if (failures.length) {
            console.error(
                `Failed to delete ${failures.length} attachment(s) from storage for advance question ${params.id}`
            );
        }
    }
