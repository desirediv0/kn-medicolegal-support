...
function AdminHistoryContent() {
        ...
        const load = async () => {
            ...
            try {
                ...
                if (data.total && currentPage > totalPages) {
                    ...
                    router.replace(
                        `/dashboard/advance-chat/history${params.toString() ? `?${params}` : ""}`
                    );
                }
            } catch (error) {
                console.error(error);
    ...
    const changePage = (nextPage) => {
        ...
        router.push(`/dashboard/advance-chat/history${params.toString() ? `?${params}` : ""}`);
    };
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-500">
                        Review all Advance Chat questions from users.
                    </p>
                </div>
            ...
            <Card>
                <CardHeader className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <Button variant="outline" size="sm" asChild>
                        <Link href="/dashboard/advance-chat">Back to Advance Chat</Link>
                    </Button>
                </CardHeader>
                                ...
                                {questions.map((question) => (
                                    ...
                                    >
                                        <div className="flex items-center justify-between gap-2">
                                            ...
                                            <Link
                                                href={`/dashboard/advance-chat?question=${question.id}`}
                                        ...
                                        {question.admin?.email && (
                                            <p className="text-xs text-gray-500 mt-1">
                                                Assigned to: {question.admin.email}
                                            </p>
                                        )}
                                    </li>
                                ))}
...
export default function AdminAdvanceChatHistory() {
    return (
